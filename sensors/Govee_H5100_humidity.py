...
class GoveeH5100Humidity:
    ...
    async def initialize(self, spriggler_logger):
        ...
        self.logger.info(
            "Govee5100Humidity sensor initialized.",
            config=self.config,
    ...
    async def start_scanning(self):
        ...
        if self.scanner is None:
            ...
            self.logger.debug(
                "Reusing existing shared BLE scanner for humidity sensor", scanner_id=id(self.scanner)
            )
    ...
    async def stop_scanning(self):
        """Stop BLE scanning."""
        if self.scanner:
            self.logger.debug("Stopping BLE scanning for humidity sensor", scanner_id=id(self.scanner))
